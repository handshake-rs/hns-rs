...
output without assuming a public-key parity, supplies the owner-bound profile's
authority adapter for the HIP-compliant HNSA service name `chat`, and encodes
bounded opaque NIP-59 gift-wrap and encrypted-acknowledgement payloads for
HIP-78 transport. The dotted `hns.chat` label belongs only to the HNSR profile
registry layer.
